...
export class IntegrationService {
   ...
   * Update integration
   */
  async updateIntegration(id: string, data: UpdateIntegrationInput): Promise<IIntegration | null> {
    if (!mongoose.Types.ObjectId.isValid(id)) {
      throw new Error('Invalid integration ID');
    }
...
<script>
  ...
  (function() {
    ...
    s.src = "${apiUrl}/api/v1/widget/loader.js"; // Update this to your CDN URL in production
    s.async = true;
