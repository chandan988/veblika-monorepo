...
import { Mail, Loader2, Trash2, Play, Square, Clock } from "lucide-react";
...
interface GmailIntegrationsProps {
  orgId: string;
}

export function GmailIntegrations({ orgId }: GmailIntegrationsProps) {
  ...
  const handleConnectGmail = async () => {
    ...
    try {
      ...
      const params = await oauth2Utils.openWithLoginUrlAndGetAllParams(authUrl, redirectUrl);
  ...
  const handleStartWatch = async (integrationId: string) => {
    ...
    try {
      ...
      toast.success("Gmail watch started! You'll receive real-time notifications.");
    } catch (error) {
      ...
      {integrations.length === 0 ? (
        <Card className="p-12 text-center">
          ...
          <p className="text-muted-foreground mb-6">
            Connect your Gmail account to start receiving emails and managing conversations
          </p>
          <Button onClick={handleConnectGmail} disabled={isConnecting}>
            {isConnecting ? (
              <>
                ...
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <h3 className="font-semibold text-lg flex items-center gap-2">
                      {integration.channelEmail || integration.credentials?.email}
                      {isWatching && (
                    ...
                    <p className="text-sm text-muted-foreground mt-1">
                      Connected {new Date(integration.createdAt || Date.now()).toLocaleDateString()}
                    </p>
                    {isWatching && watchExpiration && (
                      <p className="text-xs text-muted-foreground mt-1">
                        <Clock className="h-3 w-3 inline mr-1" />
                        {getWatchExpirationText(watchExpiration as string | Date)}
                      </p>
                    )}
                  </div>
      ...
      <AlertDialog open={!!deleteIntegrationId} onOpenChange={(open) => !open && setDeleteIntegrationId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will disconnect the Gmail account. You will stop receiving emails from this account.
